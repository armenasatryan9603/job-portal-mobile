...
export const TeamGallerySection: React.FC<TeamGallerySectionProps> = ({
  ...
  const loadGallery = async () => {
    ...
    try {
      setLoading(true);
      const items = await apiService.getTeamGallery(teamId);
      setPortfolioItems(items);
    } catch (error) {
      console.error("Error loading team gallery:", error);
    } finally {
      setLoading(false);
    }
  };
    ...
    try {
      ...
      if (!result.canceled && result.assets[0]) {
        ...
        try {
          const mediaFile = {
            uri: asset.uri,
            fileName: `team_gallery_${Date.now()}.jpg`,
            type: "image" as const,
            mimeType: "image/jpeg",
            fileSize: asset.fileSize || 0,
          };

          const uploadResult = await apiService.uploadTeamGalleryItem(
            teamId,
            {
              uri: asset.uri,
              type: "image/jpeg",
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      await apiService.updateTeamGalleryItem(
        teamId,
        editingItem.id,
        editTitle || undefined,
